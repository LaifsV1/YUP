...
let prop_entails a b = (depos_prop a = depos_prop b) (*TODO*)
...
let rec check_pf (psi : ctx) (gamma : hyps) ((pf_pos,proof) : pf) ((prop_pos,prop) : prop) :(unit option) =
  ...
  | AndL (((h',a'),(h'',b')),h,p) , c -> (match lookup_check_hyps gamma h with           (*AndL*)
                                          | Some (pos,And (a,b)) ->
                                             if (check_hyp a' a) && (check_hyp b' b)
                                             then (check_pf psi ((h',a)::(h'',b)::gamma) p (prop_pos,c))
                                             else None
                                          | _                    -> None)
  ...
  | OrL (h,((h',a'),p),((h'',b'),q)) , c -> (match lookup_check_hyps gamma h with        (*OrL*)
                                             | Some (pos,Or (a,b)) ->
                                                if (check_hyp a' a) && (check_hyp b' b)
                                                then and_also (check_pf psi ((h',a)::gamma) p (prop_pos,c))
                                                              (check_pf psi ((h'',b)::gamma) q (prop_pos,c))
                                                else None
                                             | _               -> None)
  | ImpliesR ((h,a'),p)   , Implies (a,b) -> if check_hyp a' a                           (*ImpliesR*)
                                             then check_pf psi ((h,a)::gamma) p b
                                             else None
  | ImpliesR (h,p)        , _             -> None
  | ImpliesL (p,((h',b'),h),q) , c        -> (match lookup_check_hyps gamma h with       (*ImpliesL*)
                                              | Some (pos,Implies (a,b)) ->
                                                 if (check_hyp b' b)
                                                 then and_also (check_pf psi gamma p a)
                                                               (check_pf psi ((h',b)::gamma) q (prop_pos,c))
                                                 else None
                                              | _                        -> None)
  | By h , b -> (match lookup_check_hyps gamma h with                                    (*By*)
  ...
  | ForallL ((h',a'),h,t,p)  , c -> (match lookup_check_hyps gamma h with
                                     | Some (pos,Forall (x,tau,a)) ->
                                        if check_hyp a' a
                                        then and_also (check_term psi t tau)
  ...
  | ByIndNat  (p,(n,(h,a'),q)) , Forall (m,Nat,pred) ->                                  (*ByInd-Nat*)
     ...
     if check_hyp a' pred_n
     then and_also (check_pf psi            gamma               p pred_0)
  ...
  | ByIndBool (p,q)     , Forall (b,Bool,pred)      ->                                   (*ByInd-Bool*)
     and_also (check_pf psi gamma p (subs_prop b (prop_pos,Boolean true) pred))
